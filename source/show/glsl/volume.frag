...
uniform mediump sampler3D tex3d_cube;
...
void main(void) {
	...
	if(rangeCube.x<=rangeCube.y) {
		...
		while(z<=z1) {
			...
			highp float texel=texture(tex3d_cube, tpos.xyz/tpos.w).r;
			if(texel>max_voxel) {
				max_voxel=texel;
				max_z=z;
			}
		...
		highp float a;
		if(max_voxel<xfunc_cube.x) {
			a=0.0;
		} else if(max_voxel>xfunc_cube.y) {
			a=1.0;
		} else {
			if(xfunc_cube.y-xfunc_cube.x<0.00001) {
				a=.5;
			} else {
				a=(max_voxel-xfunc_cube.x)/(xfunc_cube.y-xfunc_cube.x);
			}
		}
